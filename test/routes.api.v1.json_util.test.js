...
const { flattenAndZip, flattenJSON, zipFiles, convertToCSV, getHeaders, sanitizeFilename } = require('../routes/api/v1/json_util');

describe('flattenJSON', () => {
  it('should correctly flatten an event', () => {
    const input = [
      {
...
describe('zipFiles', () => {
  it('should return a buffer', async () => {
    const zipBuffer = await zipFiles([{ data: 'some data', name: 'events.csv' }], [{ data: 'some data', name: 'aux_data.csv' }]);

    expect(Buffer.isBuffer(zipBuffer)).to.equal(true);
  });
...
describe('sanitizeFilename', () => {
  ...
  it('should remove illegal characters', () => {
      const original = "File/with\\illegal:chars?";
      const expected = "File_with_illegal_chars_";
      expect(sanitizeFilename(original)).to.equal(expected);
  });

  it('should replace spaces with underscores', () => {
      const original = "File with spaces";
      const expected = "File_with_spaces";
      expect(sanitizeFilename(original)).to.equal(expected);
  });

  it('should not exceed maximum filename length', () => {
      const original = new Array(300).join("a");  // 299 characters of "a"
      const expected = new Array(256).join("a");  // 255 characters of "a"
      expect(sanitizeFilename(original)).to.equal(expected);
  });
});

describe('flattenAndZip', () => {
  ...
  it('should return a buffer', async () => {
        ...
        aux_data: [
          {
            ...
            data_array: [
              { data_name: "name1", data_value: "value1", data_uom: "uom1" }
            ]
          }
        ]
      }
    ...
    const result = await flattenAndZip(jsonData);
...
describe('Integration Test with Provided Input Data', () => {
  ...
  it('should correctly flatten the input data', () => {
    const flattened = flattenJSON(inputData);
    ...
    expect(flattened.events[0]['latitude (ddeg)']).to.equal("9.90565350");
  });

  it('should correctly convert the flattened data to CSV', () => {
    const { events, events_by_type } = flattenJSON(inputData);
    ...
    expect(csvAuxNavData.split('\n')[0]).to.include(["latitude (ddeg)"]);
  });

  it('should correctly zip the CSVs', async () => {
    const zipBuffer = await flattenAndZip(inputData);
