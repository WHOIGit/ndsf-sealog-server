  ...
  register: (server, options) => {
    ...
    const check_disk_space = async () => {
      ...
      await CheckDiskSpace(IMAGE_PATH).then(async (diskSpace) => {
        ...
        server.publish('/ws/status/updateCustomVars', { id: results._id, custom_var_name: results.value.custom_var_name, custom_var_value: results.value.custom_var_value } );
